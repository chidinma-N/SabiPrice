...
def load_and_clean_data():
    # Fetch the live data directly from HDX
    ...
    df_clean['mean'] = df_clean.groupby('commodity')['price_per_kg'].transform('mean')
    df_clean['std'] = df_clean.groupby('commodity')['price_per_kg'].transform('std').fillna(float('inf'))
    
    mask = (df_clean['price_per_kg'] >= df_clean['mean'] - 3 * df_clean['std']) & \
           (df_clean['price_per_kg'] <= df_clean['mean'] + 3 * df_clean['std'])
    mask = mask | (df_clean['std'] == float('inf'))
    df_clean = df_clean[mask].drop(columns=['mean', 'std'])
    
    return df_clean

def find_best_markets(df, my_lat, my_lon, commodity, max_distance_km):
    # BUG FIX 2: Removed the quotes around 'commodity' so it uses the variable instead of the string!
    ...
    # We will prioritize Wholesale prices if they exist, otherwise fallback to whatever is there (Retail)
    wholesale_df = df_target[df_target['pricetype'] == 'Wholesale']
    if not wholesale_df.empty:
        df_target = wholesale_df
        
    max_date = df_target['date'].max()
    # BUG FIX 3: Increased the cutoff to 24 months (2 years) since some WFP data is older
    recent_cutoff = max_date - pd.DateOffset(months=24) 
...
# ==========================================
# Load data first so we can use the state/market names in the dropdowns
    ...
    if search_button:
        with st.spinner("Searching for the highest prices nearby..."):
            ...
            if isinstance(report, str):
                ...
                top_type = report.iloc[0]['Market Type']
                
                st.info(f"💡 **FARMER'S ADVICE:** People are buying {selected_crop} for **₦{top_price} per KG** ({top_type} price) in {top_market}, {top_state} right now. If a middleman comes to your farm offering much less than this, do not accept it!")
    else:
